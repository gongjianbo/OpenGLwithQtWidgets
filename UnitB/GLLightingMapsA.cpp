#include "GLLightingMapsA.h"

#include <QDebug>

GLLightingMapsA::GLLightingMapsA(QWidget *parent)
...
GLLightingMapsA::~GLLightingMapsA()
{
...
void GLLightingMapsA::initializeGL()
{
    //为当前上下文初始化OpenGL函数解析
    ...
    _textureBox=new QOpenGLTexture(QImage(":/box2.png"),QOpenGLTexture::GenerateMipMaps);
    if(!_textureBox->isCreated()){
        qDebug() << "Failed to load texture";
    }
    //set the texture wrapping parameters
    //等价于glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    _textureBox->setWrapMode(QOpenGLTexture::DirectionS, QOpenGLTexture::Repeat);
    _textureBox->setWrapMode(QOpenGLTexture::DirectionT, QOpenGLTexture::Repeat);
    // set texture filtering parameters
    //等价于glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    _textureBox->setMinificationFilter(QOpenGLTexture::Linear);
    _textureBox->setMagnificationFilter(QOpenGLTexture::Linear);
...
void GLLightingMapsA::paintGL()
{
    ...
    _cubeShader.setUniformValue("material.shininess",32.0f);
...
void GLLightingMapsA::initShader()
{
    //cube shader
